...
public class SequenceLastFunctionBehaviorExecution implements OpaqueBehaviorExecution {
    ...
    public void doBody(List<ParameterValue> inputs, List<ParameterValue> outputs, Debug debug) {
        ...
        if (!seq.isEmpty()) {
            Value result = seq.get(seq.size() - 1);
            debug.println("[doBody] Last result = " + result);            
    ...
    public OpaqueBehaviorExecution new_() {
        return new SequenceLastFunctionBehaviorExecution();
    }

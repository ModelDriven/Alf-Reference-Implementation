...
public class SequenceIncludingFunctionBehaviorExecution implements OpaqueBehaviorExecution {
    ...
    public void doBody(List<ParameterValue> inputs, List<ParameterValue> outputs, Debug debug) {
        ...
        final List<? extends Value> optElement = inputs.get(1).getValues();
        
        outputs.get(0).addValues(seq);
        
        if (!optElement.isEmpty()) {
            final Value element = optElement.get(0);
            debug.println("[doBody] Including element = " + element);
            outputs.get(0).addValue(element);
        }
    }

    @Override
    public OpaqueBehaviorExecution new_() {
        return new SequenceIncludingFunctionBehaviorExecution();
    }

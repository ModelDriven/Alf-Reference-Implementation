...
Prints an indented textual representation of the abstract syntax tree for the
input file. The input file must contain a single Alf unit.

The "-complete" option results in the completion of all imports and stubs.
Prints the abstract syntax tree beginning at the root namespace, rather than the
input unit.

Imported units are expected to be found in appropriate subdirectories of either
Root\Model (for model units) or Root\Library (for library units). Subdirectory 
